...
//	glm::vec3 refl_color = glm::vec3(0.f);
//	glm::vec3 half_vec;
//	glm::vec3 diffuse_specular_sum = glm::vec3(0.f);
	...
	//			is_visible = trace_shadow(light_direction, out_prim_int, out_primitive_hit, scene_primitives, dist_to_light);
			...
			if (depth < scene.max_depth)
			{
				...
				glm::vec3 refl_color = (out_primitive_hit->prim_specular)*(compute_pixel_color(refl_ray, scene_primitives, scene_lights, scene, depth + 1));
...
//	float shadow_bias = .0001f;
	...
	float temp_out_t;
	return !(trace_ray_to_primitive(shadow_ray, scene_primitives, temp_out_t, temp_prim, temp_prim_int));
